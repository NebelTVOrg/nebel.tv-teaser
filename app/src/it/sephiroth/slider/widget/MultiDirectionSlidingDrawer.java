package it.sephiroth.slider.widget;
...
public class MultiDirectionSlidingDrawer extends ViewGroup {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mTracking || mAnimating) {
			...
			if (cache != null) {
				...
				canvas.save();
				if (mInvert) {
					canvas.translate(isVertical ? 0 : handle.getLeft() - mTopOffset - mContent.getMeasuredWidth(), isVertical ? handle.getTop()
							- mTopOffset - mContent.getMeasuredHeight() : 0);
				} else {
	...
	private void performFling(int position, float velocity, boolean always) {
		...
		if (mExpanded) {
			...
			c2 = mInvert ? (bottom - (position + handleHeight)) + mBottomOffset > handleHeight : position > mTopOffset
